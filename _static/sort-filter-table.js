...
var Sort = (function(){
	...
	// Invert string (for use when combining descending with ascending sorts)
	sort.invert_string = function(s) {
		var t = "";
		var zEncoding = "z".charCodeAt(0);
		for (var i=0, L=s.length; i<L; i++) {
			t += String.fromCharCode(zEncoding - s.charCodeAt(i));
		}
		return t;
	};
...
var Table = (function(){
	...
	var table = {
		...
		SecondarySortColumnPrefix:"table-secondary:",
	...
	table.sort = function(o,args) {
		...
		var col = tdata.col;
		var invert_string = Sort["invert_string"];
				...
				if (cRow=tbrows[cRowIndex]){
					...
					do {
						if (rowCells = cRow.cells) {
							...
							// Always secondary sort with first column.
							if (cellValue && tdata.secondarySortColumn != null) {
								var col1value = this.getCellValue(rowCells[tdata.secondarySortColumn],useinnertext);
								if (tdata.desc) {
									cellValue += invert_string(col1value);
								} else {
									cellValue += col1value;
								}
							}
	...
	table.auto = function(args) {
		...
		if (tables!=null) {
			for (var i=0,L=tables.length; i<L; i++) {
				...
				val = classValue(t,table.SecondarySortColumnPrefix);
				tdata.secondarySortColumn = (val==null)?null:+val;
